...
CONFIG_KEY_ROUTING_RULES = "routing_rules"
CONFIG_KEY_EXIT = "exit"
CONFIG_KEY_MODE = "mode"
CONFIG_KEY_SRC_IP = "src_ip"
...
ROUTING_MODES = {ROUTING_MODE_WAN, ROUTING_MODE_SPLIT, ROUTING_MODE_EXIT}
...
# AWG v3.1.20260827 fixes kernel header-protection key setup/locking.
# Header protection requires every S1-S4 padding value to be at least the
# 12-byte header-protection nonce size. Keep the protocol-level accepted
# ranges above unchanged: explicit configs without HPK may still use < 12.
AWG_HEADER_PROTECTION_MIN_PADDING = 12
AWG_HEADER_PROTECTION_ENABLED = True

# Deterministic infra auto-generation ranges. When HPK is generated by
# default, draw S1-S4 directly from a valid range instead of drawing from
# 1..MAX and clamping low values afterwards.
...
AWG_INFRA_AUTO_S1_MIN = (
    max(AWG_S1_MIN, AWG_HEADER_PROTECTION_MIN_PADDING)
    if AWG_HEADER_PROTECTION_ENABLED
    else AWG_S1_MIN
)
AWG_INFRA_AUTO_S1_MAX = AWG_S1_MAX
AWG_INFRA_AUTO_S2_MIN = (
    max(AWG_S2_MIN, AWG_HEADER_PROTECTION_MIN_PADDING)
    if AWG_HEADER_PROTECTION_ENABLED
    else AWG_S2_MIN
)
AWG_INFRA_AUTO_S2_MAX = AWG_S2_MAX
AWG_INFRA_AUTO_S3_MIN = (
    max(AWG_S3_MIN, AWG_HEADER_PROTECTION_MIN_PADDING)
    if AWG_HEADER_PROTECTION_ENABLED
    else AWG_S3_MIN
)
AWG_INFRA_AUTO_S3_MAX = AWG_S3_MAX
AWG_INFRA_AUTO_S4_MIN = (
    max(AWG_S4_MIN, AWG_HEADER_PROTECTION_MIN_PADDING)
    if AWG_HEADER_PROTECTION_ENABLED
    else AWG_S4_MIN
)
...
TOPOLOGY_2D_OUT = "topology/topology-2d.html"
TOPOLOGY_2D_SVG_OUT = "topology/topology-2d.svg"
TOPOLOGY_3D_OUT = "topology/topology-3d.html"
