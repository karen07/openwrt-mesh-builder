...
Dynamic direct-list sources из `tools/default.py` должны быть доступны, если они включены. Если нужен полностью локальный smoke-run без загрузки country/ASN direct-list IP sets, добавь `--skip-direct-downloads`:

```bash
./generate_configs.py --skip-awg-download --skip-package-sync --skip-direct-downloads
```

В этом режиме generated `direct.txt` будет содержать только static direct entries: `LOCAL_DIRECT_IPSETS`, `EXIT_DIRECT_STATIC_IPSETS`, listen IP mesh/exit hubs и exit IP.
...
`--skip-hooks` пропускает запуск `tools.generate`, `tools.ensure_ssh_keys`, validation hook из `tools.validate` и `tools.show_unmanaged`.
...
Результат складывается в `images/`. Набор install-образов зависит от OpenWrt device profile: обычно есть `sysupgrade`, а `factory` появляется только для профилей, где его генерирует ImageBuilder.
...
wg
openssl
apk-tools 3.x (`apk`; нужен `apk adbdump` или `apk manifest`)
